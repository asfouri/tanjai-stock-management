...
import { getSessionWithTimeout, setCachedAccessToken } from "../dashboard/api";
...
export function LoginClient({
  ...
  const queryLoginAttemptedRef = useRef(false);
  ...
  useEffect(() => {
    ...
    async function checkSession() {
      try {
        ...
        console.debug("Saved session check skipped.", error);
      }
    }
  ...
  useEffect(() => {
    if (
      queryLoginAttemptedRef.current ||
      !shouldAutoSubmit ||
      !initialEmail ||
      !initialPassword
    ) {
    ...
    queryLoginAttemptedRef.current = true;
    window.history.replaceState(null, "", window.location.pathname);
    const form = document.querySelector<HTMLFormElement>("#login-form");
    window.setTimeout(() => form?.requestSubmit(), 0);
  }, [initialEmail, initialPassword, shouldAutoSubmit, signIn]);

...
import { diffByKey, diffMultiset, hashContent } from '../utilities/refresh-diff';
...
type ObservedSharedFile = {
  signature: string;
  stableSince: number;
  processedSignature?: string;
};

const defaultAutoRefreshPollIntervalSeconds = 60;
const defaultMinAutoRefreshIntervalSeconds = 12 * 60 * 60;

type IncrementalRefreshChanges = {
  orders: { inserted: number; updated: number; deleted: number };
};

// Thrown to abort (roll back) a benchmark transaction after the work is done,
// carrying the change counts back out so nothing is persisted.
class DryRunRollback extends Error {
  constructor(readonly changes: IncrementalRefreshChanges) {
    super('benchmark dry-run rollback');
  }
}

@Injectable()
export class ExcelImportService implements OnModuleInit, OnModuleDestroy {
  private autoRefreshTimer?: NodeJS.Timeout;
  private autoRefreshCheckRunning = false;
  private readonly activeAutomaticRefreshes = new Set<string>();
  private readonly observedSharedFiles = new Map<string, ObservedSharedFile>();
  private readonly lastAutomaticRefreshAt = new Map<string, number>();
  ...
  onModuleInit() {
    void this.detachStoreAssociationsFromDeposits();

    if (
      process.env.EXCEL_AUTO_REFRESH?.trim().toLowerCase() !== 'true' ||
      process.env.EXCEL_IMPORT_STORAGE !== 'prisma' ||
      !process.env.EXCEL_IMPORT_DIR?.trim()
    ) {
      return;
    }
    ...
    const intervalSeconds = Math.max(
      5,
      Number(process.env.EXCEL_REFRESH_INTERVAL_SECONDS) ||
        defaultAutoRefreshPollIntervalSeconds,
    );
    this.autoRefreshTimer = setInterval(() => {
      void this.checkSharedFilesForUpdates();
  ...
  private async detachStoreAssociationsFromDeposits() {
    try {
      ...
      console.error(
        '[excel-import] unable to detach legacy deposit store associations:',
  ...
  async preview(
    ...
    options: { skipDuplicateLookup?: boolean } = {},
    ...
    try {
      ...
      const duplicateRecords = options.skipDuplicateLookup
        ? []
        : await this.findDuplicateRecords(parsed);
  ...
  async confirm(token: string, replaceImportBatchId?: string) {
    if (!token) {
      ...
      result = await this.prisma.$transaction(
        async (tx) => {
          ...
          await tx.$executeRawUnsafe(
            'SET LOCAL idle_in_transaction_session_timeout = 280000',
          );

          if (replaceImportBatchId) {
            const batchToReplace = await tx.importBatch.findUnique({
              where: { id: replaceImportBatchId },
              select: { fileName: true },
            });
            if (!batchToReplace) {
              throw new NotFoundException('Import batch was not found.');
            }
            await this.deleteImportBatchRecords(
              tx,
              replaceImportBatchId,
              batchToReplace.fileName,
            );
            logPhase('previous batch removed');
          }
          ...
          const { storeIds, productIds, skuProductIds } =
            await this.resolveImportContext(tx, entry.parsed);
          logPhase(
            `stores ${entry.parsed.stores.length}, products ${entry.parsed.products.length}`,
          );
          ...
          await this.createManyInChunks(
            ...
            entry.parsed.walletTransactions.map((transaction) => {
              ...
              return {
                importBatchId: batch.id,
                storeId:
                  !isDeposit && transaction.storeName
    ...
    void this.track17Service
      .registerByImportBatch(result.importBatchId)
      .catch((error: unknown) => {
        console.warn(
          `[excel-import] 17TRACK registration could not complete for import ${result.importBatchId}: ${error instanceof Error ? error.message : 'unknown error'}`,
        );
      });
  ...
  async replaceImportBatch(importBatchId: string, token: string) {
    ...
    if (existingBatch.fileHash === entry.parsed.fileHash) {
      previewStore.delete(token);
      return {
        importBatchId: existingBatch.id,
        summary: entry.preview,
        ignored: true,
        message: `${existingBatch.fileName} is already up to date. No data was changed.`,
      };
    }

    return this.confirm(token, importBatchId);
  }

  /**
   * Upsert the brand, stores and products for an import and return the id maps
   * the row builders need. Shared by the full-import (confirm) and incremental
   * refresh paths so both resolve identities identically.
   */
  private async resolveImportContext(
    tx: any,
    parsed: ParsedImport,
    options: { createOnly?: boolean } = {},
  ) {
    // createOnly (used by incremental refresh): only read existing brand /
    // stores / products and create the ones that are genuinely new. It skips
    // the per-row metadata re-writes that a full import does, which is what
    // makes a refresh fast — the brand/store/product catalog rarely changes
    // between refreshes, and a full re-import still syncs metadata edits.
    const createOnly = options.createOnly ?? false;
    const brandName = parsed.brandName || 'TanjAI';
    const brand =
      (createOnly
        ? await tx.brand.findUnique({ where: { name: brandName } })
        : null) ??
    ...
    if (createOnly) {
      const existingStores: Array<{ id: string; normalizedName: string }> =
        await tx.store.findMany({
          where: { brandId: brand.id },
          select: { id: true, normalizedName: true },
        });
      const storeIdByNorm = new Map<string, string>(
        existingStores.map((s) => [s.normalizedName, s.id]),
      );
      for (const store of parsed.stores) {
        let id = storeIdByNorm.get(store.normalizedName);
        if (!id) {
          ...
          storeIdByNorm.set(store.normalizedName, id);
        }
        storeIds.set(store.name, id);
        storeIds.set(store.normalizedName, id);
      }
    } else {
      for (const store of parsed.stores) {
    ...
    return { brandId: brand.id, storeIds, productIds, skuProductIds };
  }
  ...
  private orderKey(
    ...
    return `${storeId} ${externalOrderNumber} ${invoiceReference}`;
  }

  /**
   * Canonical shape of an order used for change detection. Includes the order's
   * lines and its set of tracking numbers so a change to any child row marks
   * the order as changed. Must produce the same structure for a parsed order
   * and for an order loaded back from the database.
   */
  private orderHashInput(order: {
    orderDate?: Date | null;
    country?: string | null;
    status: string;
    sourceSheet: string;
    ...
    // Sort lines into a canonical order: the database returns an order's lines
    // in no guaranteed order, while parsed lines follow sheet order, so hashing
    // them positionally would flag unchanged orders as changed. Sorting both
    // sides identically makes the hash depend only on line content.
    const lines = order.lines
      ...
      .sort((a, b) => (hashContent(a) < hashContent(b) ? -1 : 1));
    return {
      ...
      sourceRow: order.sourceRow,
      lines,
      trackingNumbers: [...order.trackingNumbers].sort(),
    };
  }
  ...
  /**
   * Diff a batch-scoped table that has no stable business key by matching rows
   * on a content hash (multiset). Only rows that changed are written.
   */
  private async diffLeafTable(
    tx: any,
    table: string,
    ...
    for (const key of shapeKeys) select[key] = true;
    ...
    const existingHashed = existing.map((row) => ({
      id: row.id,
      hash: hashContent(this.pick(row, shapeKeys)),
    }));
    const incomingHashed = incomingRows.map((row, index) => ({
      hash: hashContent(this.pick(row, shapeKeys)),
      index,
    }));

    const { toInsertIndexes, toDeleteIds } = diffMultiset(
      existingHashed,
      incomingHashed,
    );

    for (const chunk of this.chunk(toDeleteIds, 1000)) {
      if (chunk.length) {
        await delegate.deleteMany({ where: { id: { in: chunk } } });
      }
    }
    const insertRows = toInsertIndexes.map((index) => incomingRows[index]);
    await this.createManyInChunks(delegate, insertRows, skipDuplicates);

    return { inserted: insertRows.length, deleted: toDeleteIds.length };
  }

  /**
   * Refresh an existing import batch in place by diffing the newly-parsed
   * workbook against what is already stored, writing only the rows that
   * changed. Unlike the full replace path this keeps the same batch id and,
   * crucially, preserves shipments (and their 17TRACK tracking state) whose
   * order is unchanged. Runs in a single transaction; on any failure the caller
   * falls back to the full replace path.
   */
  async applyIncrementalRefresh(importBatchId: string, token: string) {
    const entry = previewStore.get(token);
    if (!entry) {
      throw new NotFoundException('Import preview expired or was not found.');
    }
    ...
    const changes = await this.runIncrementalRefreshTx(
      importBatchId,
      parsed,
      preview,
      {},
    ...
    console.info(
      `[excel-import] INCREMENTAL refresh total: ${Date.now() - startedAt}ms ` +
        `(orders +${changes.orders.inserted} ~${changes.orders.updated} -${changes.orders.deleted})`,
    );

    void this.track17Service
      .registerByImportBatch(importBatchId)
      .catch((error: unknown) => {
        console.warn(
          `[excel-import] 17TRACK registration could not complete for refreshed import ${importBatchId}: ${error instanceof Error ? error.message : 'unknown error'}`,
        );
      });
    ...
    return {
      ...
      incremental: true,
      changes,
      message: `${parsed.fileName} refreshed. Orders: +${changes.orders.inserted} / ~${changes.orders.updated} / -${changes.orders.deleted}.`,
    };
  }

  private async runIncrementalRefreshTx(
    importBatchId: string,
    parsed: ParsedImport,
    preview: ImportPreview,
    options: { simulateFullReplace?: boolean; dryRun?: boolean },
  ): Promise<IncrementalRefreshChanges> {
    return this.prisma.$transaction(
      async (tx: any) => {
        await tx.$executeRawUnsafe('SET LOCAL statement_timeout = 280000');

        let tracePrev = Date.now();
        const trace = (label: string) => {
          if (!options.dryRun) return;
          const now = Date.now();
          console.info(`[bench]   ${label}: ${now - tracePrev}ms`);
          tracePrev = now;
        };

        // Benchmark only: wipe the batch's child rows first so the diff below
        // treats every row as an insert, reproducing full-replace write volume.
        if (options.simulateFullReplace) {
          await this.deleteImportBatchChildRecords(tx, importBatchId);
          trace('simulateFullReplace delete');
        }
        ...
        await tx.importBatch.update({
          ...
          data: {
            ...
            summary: this.previewSummary(preview),
        ...
        const { storeIds, productIds, skuProductIds } =
          await this.resolveImportContext(tx, parsed, { createOnly: true });
        trace('resolveImportContext');

        // ---- Orders (+ lines, + shipments): diff by natural key ----
        const incomingOrders = parsed.orders.map((order) => {
          const storeId = this.requireStoreId(storeIds, order.storeName);
          const seenTracking = new Set<string>();
          ...
          for (const shipment of order.shipments) {
            if (seenTracking.has(shipment.trackingNumber)) continue;
            seenTracking.add(shipment.trackingNumber);
            shipments.push({
              trackingNumber: shipment.trackingNumber,
              sourceSheet: shipment.sourceSheet,
        ...
        const existingOrders = (await tx.order.findMany({
          ...
          select: {
            ...
            shipments: { select: { trackingNumber: true } },
          },
        })) as any[];

        const existingKeyed = existingOrders.map((order) => ({
          ...
          hash: hashContent(
            this.orderHashInput({
              ...order,
              trackingNumbers: order.shipments.map(
                (s: { trackingNumber: string }) => s.trackingNumber,
              ),
            }),
          ),
        }));
        const incomingKeyed = incomingOrders.map((order) => ({
          key: this.orderKey(
            order.storeId,
            order.externalOrderNumber,
            order.invoiceReference,
          ),
          hash: hashContent(this.orderHashInput(order)),
          data: order,
        }));

        trace('load existing orders + hash/diff');
        const orderDiff = diffByKey(existingKeyed, incomingKeyed);

        // Deleted orders cascade to their lines, shipments and tracking events.
        for (const chunk of this.chunk(orderDiff.toDeleteIds, 1000)) {
          if (chunk.length) {
            await tx.order.deleteMany({ where: { id: { in: chunk } } });
          }
        }

        // Inserted orders: create the order rows, then lines and shipments only
        // for the ones that actually persisted (the order natural key is global,
        // so a collision with another batch is skipped rather than errored).
        const insertMeta = orderDiff.toInsert.map((order) => ({
          id: randomUUID(),
          order,
        }));
        await this.createManyInChunks(
          tx.order,
          insertMeta.map(({ id, order }) => ({
            id,
            importBatchId,
        ...
        for (const chunk of this.chunk(
          ...
          for (const row of rows) persistedOrderIds.add(row.id);
        }
        const newLineRows: Array<Record<string, unknown>> = [];
        const newShipmentRows: Array<Record<string, unknown>> = [];
        for (const { id, order } of insertMeta) {
          if (!persistedOrderIds.has(id)) continue;
          for (const line of order.lines) {
            newLineRows.push({
              orderId: id,
              productId: this.lookupSkuProductId(skuProductIds, line.sku),
        ...
        await this.createManyInChunks(tx.shipment, newShipmentRows, true);
        await this.createManyInChunks(tx.orderLine, newLineRows);

        // Changed orders: update scalar fields, replace lines, and reconcile
        // shipments by tracking number so existing shipments keep their tracking
        // state (only genuinely new tracking numbers are added).
        for (const { id, data: order } of orderDiff.toUpdate) {
          ...
          })) as Array<{ id: string; trackingNumber: string }>;
          const existingTracking = new Set(
            existingShipments.map((s) => s.trackingNumber),
          );
          const incomingTracking = new Set(order.trackingNumbers);
          const shipmentsToRemove = existingShipments
            .filter((s) => !incomingTracking.has(s.trackingNumber))
            .map((s) => s.id);
          if (shipmentsToRemove.length) {
            await tx.shipment.deleteMany({
              where: { id: { in: shipmentsToRemove } },
            });
          }
          const shipmentsToAdd = order.shipments.filter(
            (s) => !existingTracking.has(s.trackingNumber),
          );
          if (shipmentsToAdd.length) {
            await this.createManyInChunks(
              tx.shipment,
        ...
        trace('apply order inserts/updates/deletes');

        // ---- Fulfillment invoices ----
        await this.diffLeafTable(
          tx,
          'fulfillmentInvoice',
          importBatchId,
          parsed.invoices.map((invoice) => ({
            importBatchId,
            storeId: this.requireStoreId(storeIds, invoice.storeName),
            invoiceReference: invoice.invoiceReference,
            invoiceDate: invoice.invoiceDate,
            subtotal: invoice.subtotal,
            refunds: invoice.refunds,
        ...
        await this.diffLeafTable(
          ...
          'stockPurchase',
          importBatchId,
          parsed.stockPurchases.map((purchase) => ({
            importBatchId,
            productId: purchase.productName
              ? this.lookupProductId(productIds, purchase.productName)
              : purchase.sku
        ...
        await this.diffLeafTable(
          ...
          'inventoryMovement',
          importBatchId,
          parsed.inventoryMovements.map((movement) => ({
            importBatchId,
            productId: movement.productName
              ? this.lookupProductId(productIds, movement.productName)
              : undefined,
        ...
        trace('leaf diff: invoices/purchases/movements');

        // ---- Inventory items + product links ----
        // These are few (one per stock name) and their admin-confirmed mapping
        // is re-derived from the global ProductAlias table, so a rebuild is both
        // cheap and non-destructive to confirmations.
        const inventoryItemIds = (
          await tx.inventoryItem.findMany({
            where: { importBatchId },
            select: { id: true },
        ...
        await this.diffLeafTable(
          ...
          'walletTransaction',
          importBatchId,
          parsed.walletTransactions.map((transaction) => {
            const isDeposit = transaction.transactionType
              .toLowerCase()
              .includes('deposit');
            return {
        ...
        await this.diffLeafTable(
          ...
          ['severity', 'message', 'sourceSheet', 'sourceRow'],
        );

        trace('wallet + anomalies');

        const result = {
          orders: {
            inserted: orderDiff.toInsert.length,
            updated: orderDiff.toUpdate.length,
            deleted: orderDiff.toDeleteIds.length,
          },
        };
        if (options.dryRun) {
  ...
  private async deleteImportBatchChildRecords(
    tx: any,
    importBatchId: string,
  ) {
    const inventoryItemIds = (
  ...
  /**
   * Parse a workbook into the same { parsed, preview } the import pipeline uses,
   * without touching the database or the preview cache. For the benchmark tool.
   */
  async parseFileForBenchmark(file: UploadedExcelFile) {
    const buffer = file.buffer ?? (await readFile(file.path ?? ''));
    const fileHash = createHash('sha256').update(buffer).digest('hex');
    const workbook = parseXlsxWorkbook(buffer);
    const parsed = this.parseWorkbook(workbook, file.originalname, fileHash);
    const preview = this.buildPreview(randomUUID(), parsed, []);
    return { parsed, preview };
  }

  /**
   * Time a full-replace-equivalent refresh against an incremental refresh for
   * the same file. Both run inside transactions that are rolled back, so nothing
   * is persisted. Used by scripts/benchmark-refresh.mjs.
   */
  async benchmarkRefresh(
    importBatchId: string,
    parsed: ParsedImport,
    preview: ImportPreview,
    options: { skipFullReplace?: boolean } = {},
  ) {
    this.ensureTransactionCompatibleDatabaseUrl();
    await this.ensureDatabaseReady();

    const timeRun = async (options: {
      simulateFullReplace?: boolean;
    }): Promise<{ ms: number; changes: IncrementalRefreshChanges }> => {
      const startedAt = Date.now();
      try {
        const changes = await this.runIncrementalRefreshTx(
          importBatchId,
        ...
        return { ms: Date.now() - startedAt, changes };
      } catch (error) {
        if (error instanceof DryRunRollback) {
          return { ms: Date.now() - startedAt, changes: error.changes };
        }
        throw error;
      }
    };

    const fullReplace = options.skipFullReplace
      ? null
      : await timeRun({ simulateFullReplace: true });
    const incremental = await timeRun({});
    return { fullReplace, incremental };
  }

  async refreshImportBatch(importBatchId: string) {
    if (!importBatchId) {
      throw new BadRequestException('Import batch ID is required.');
    }
    ...
    if (process.env.EXCEL_IMPORT_STORAGE !== 'prisma') {
      throw new BadRequestException(
        'Excel refresh is only available when EXCEL_IMPORT_STORAGE=prisma.',
      );
    }
    ...
    if (!importDir) {
      throw new BadRequestException(
        'EXCEL_IMPORT_DIR must point to the synchronized shared Excel folder.',
      );
    }

    const existingBatch = await this.prisma.importBatch.findUnique({
      where: { id: importBatchId },
      select: { id: true, fileName: true, fileHash: true, summary: true },
    });
    if (!existingBatch) {
      throw new NotFoundException('Import batch was not found.');
    }

    const configuredImportDir = resolve(importDir);
    const safeName = basename(existingBatch.fileName);
    const filePath = resolve(configuredImportDir, safeName);
    const buffer = await readFile(filePath).catch(() => {
      throw new BadRequestException(
        `Excel file was not found in the shared folder: ${safeName}`,
      );
    });
    const fileHash = createHash('sha256').update(buffer).digest('hex');

    if (fileHash === existingBatch.fileHash) {
      return {
        importBatchId: existingBatch.id,
    ...
    // Incremental refresh diffs the workbook against what is already stored and
    // writes only the rows that changed, keeping the same import batch. This
    // makes each refresh transaction short (dashboard reads are not blocked)
    // and preserves shipment tracking state instead of wiping and re-registering
    // every shipment. It is opt-in until validated against production data; any
    // failure falls back to the proven full-replace path.
    if (process.env.EXCEL_INCREMENTAL_REFRESH === 'true') {
      try {
        return await this.applyIncrementalRefresh(
          existingBatch.id,
          preview.token,
        );
      } catch (error) {
        console.error(
          `[excel-import] incremental refresh failed for ${safeName}, falling back to full replace:`,
          error instanceof Error ? error.message : error,
        );
      }
    }

    return this.replaceImportBatch(existingBatch.id, preview.token);
  }

  startRefreshImportBatch(importBatchId: string) {
    if (!importBatchId) {
      throw new BadRequestException('Import batch ID is required.');
    }

    const activeJob = refreshJobs.get(importBatchId);
    if (activeJob?.status === 'RUNNING') {
      return {
        importBatchId,
        ...activeJob,
        message: 'A refresh is already running in the background.',
      };
    }
    ...
    refreshJobs.set(importBatchId, job);

    setImmediate(() => {
      void this.refreshImportBatch(importBatchId)
        .then((result) => {
          const refreshResult = result as {
            ignored?: boolean;
            message?: string;
          };
          refreshJobs.set(importBatchId, {
            status: 'COMPLETED',
            startedAt: job.startedAt,
            completedAt: new Date().toISOString(),
            ignored: refreshResult.ignored,
            message:
              refreshResult.message ?? 'Excel data refreshed successfully.',
          });
        })
        .catch((error: unknown) => {
          refreshJobs.set(importBatchId, {
    ...
    return { importBatchId, ...job };
  }

  getRefreshImportBatchStatus(importBatchId: string) {
    if (!importBatchId) {
      throw new BadRequestException('Import batch ID is required.');
    }
  ...
  private async checkSharedFilesForUpdates() {
    if (this.autoRefreshCheckRunning) return;
    this.autoRefreshCheckRunning = true;

    try {
      const importDir = process.env.EXCEL_IMPORT_DIR?.trim();
      if (!importDir) return;

      const stabilitySeconds = Math.max(
        5,
        Number(process.env.EXCEL_STABILITY_SECONDS) || 10,
      );
      ...
      for (const batch of batches) {
        const safeName = basename(batch.fileName);
        const filePath = resolve(importDir, safeName);
        const fileStats = await stat(filePath).catch(() => null);
        if (!fileStats?.isFile()) continue;

        const signature = `${fileStats.size}:${fileStats.mtimeMs}`;
        const observed = this.observedSharedFiles.get(safeName);
        if (!observed || observed.signature !== signature) {
          this.observedSharedFiles.set(safeName, {
            signature,
            stableSince: Date.now(),
            processedSignature: observed?.processedSignature,
          });
          continue;
        }

        const isStable =
          Date.now() - observed.stableSince >= stabilitySeconds * 1000;
        const manualJobRunning =
          refreshJobs.get(batch.id)?.status === 'RUNNING';
        if (
          !isStable ||
          observed.processedSignature === signature ||
          manualJobRunning ||
          this.activeAutomaticRefreshes.has(safeName)
        ) {
          continue;
        }

        // Check the shared folder frequently enough to notice a stable save,
        // but apply automatic changes at most once every 12 hours by default.
        // The manual "Refresh now" action bypasses this interval and stays
        // instant. We deliberately do NOT mark processedSignature here, so the
        // latest change is retried once the 12-hour window elapses.
        const minIntervalMs =
          Math.max(
            0,
            Number(process.env.EXCEL_MIN_AUTO_REFRESH_INTERVAL_SECONDS) ||
              defaultMinAutoRefreshIntervalSeconds,
          ) * 1000;
        const lastRefreshAt = this.lastAutomaticRefreshAt.get(safeName) ?? 0;
        if (minIntervalMs > 0 && Date.now() - lastRefreshAt < minIntervalMs) {
          continue;
        }

        this.activeAutomaticRefreshes.add(safeName);
        this.lastAutomaticRefreshAt.set(safeName, Date.now());
        void this.refreshImportBatch(batch.id)
          .then(() => {
            const current = this.observedSharedFiles.get(safeName);
            if (current?.signature === signature) {
              current.processedSignature = signature;
            }
          })
          .catch((error: unknown) => {
            console.error(
              `[excel-import] automatic refresh failed for ${safeName}:`,
              error instanceof Error ? error.message : error,
            );
          })
          .finally(() => {
            this.activeAutomaticRefreshes.delete(safeName);
          });
      }
    } catch (error) {
      console.error(
        '[excel-import] automatic refresh check failed:',
        error instanceof Error ? error.message : error,
      );
    } finally {
      this.autoRefreshCheckRunning = false;
    }
  }

  private async deleteImportBatchRecords(
    tx: any,
    importBatchId: string,
    fileName: string,
  ) {
  ...
  private async resolveImportedProducts(
    ...
    for (const alias of aliasRows) {
      aliasesBySku.set(alias.sku, {
        ...alias,
        product: productsById.get(alias.productId),
      });
    ...
    for (const product of orderedProducts) {
      ...
      if (!saved) {
        ...
        if (
          ...
          saved = await this.updateProductMetadata(tx, saved, product, false);
        }
      }
      // createOnly && existing product: keep it as-is (no metadata write).
      ...
      if (sku) {
        ...
        // Skip the upsert when the alias already points at the right product
        // and store — the common case on a refresh, and another ~200 avoided
        // round-trips.
        const aliasIsCurrent =
          existingAlias?.sku === sku &&
          existingAlias?.productId === saved.id &&
          (existingAlias?.storeId ?? null) === (aliasStoreId ?? null);
        // In createOnly (refresh) mode, only create an alias that is missing
        // entirely; never rewrite an existing one.
        const shouldWriteAlias =
          !aliasIsCurrent && (!createOnly || !existingAlias);
        if (shouldWriteAlias) {
  ...
  private async updateProductMetadata(
    ...
    if (updateName && product.name !== existing.name) {
    ...
    if (product.quotation) {
      ...
      const mergedQuotation =
        mergeProductQuotation(
          existing.quotation as ParsedProduct['quotation'],
          product.quotation,
        ) ?? product.quotation;
      if (
        JSON.stringify(mergedQuotation) !== JSON.stringify(existing.quotation)
      ) {
        data.quotation = mergedQuotation;
      }
    }
    ...
    // Nothing changed: return the row we already have without touching the
    // database. On a refresh almost every product is unchanged, so skipping
    // these per-product writes is what keeps product resolution fast.
  ...
  private throwConfirmImportError(error: unknown): never {
    ...
    if (details.includes('does not exist') || details.includes('relation')) {
      throw new BadRequestException(
        'Cannot access the import database because the Excel import tables do not exist yet. Run backend/prisma/excel-import-tables.sql in Supabase SQL Editor.',
      );
    }
  ...
  private async ensureDatabaseReady() {
    try {
      ...
      if (
        ...
        throw new BadRequestException(
          'Cannot access the import database because the Excel import tables do not exist yet. Open Supabase SQL Editor and run backend/prisma/excel-import-tables.sql. Do not run prisma migrate reset or prisma db push on this existing database.',
        );
      }
